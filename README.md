...
argobot runs a Kubernetes informer over Argo CD Application CRs. Kubernetes watches cannot filter on annotations, so it watches every Application in the namespace and maps each to its deployment Space in-process: an Application's OCI source (`.../space/<slug>`) — equivalently its name — names the Space, and the slug is resolved to a Space id by a direct lookup. The worker may read a Space it is the release bridge worker for — exactly the deployment Spaces it reports on — so a slug that resolves is one argobot is responsible for; a slug that does not resolve is ignored. For each mapped Application, argobot projects a concise status — sync status, health status, operation phase, synced revision, a short message — and merge-patches it onto the Space as the `confighub.com/live-status` annotation. The worker identity is authorized to write for the same reason it can read: it is the Space's release bridge worker.
...
Every image carries its third-party notices at `/app/THIRD_PARTY_LICENSES.txt` (Go modules linked into the binary) and `/app/OS_LICENSE_NOTICE.txt` (Alpine base packages, with the GPL source offer), and every published manifest has an SPDX SBOM attestation (`docker buildx imagetools inspect ghcr.io/confighub/argobot:<tag> --format '{{json .SBOM}}'`). After changing dependencies, run `./scripts/gen-third-party-licenses.sh` and commit the result; `.github/workflows/third-party-licenses.yml` fails the PR if the committed file is stale or a copyleft dependency is linked in.
